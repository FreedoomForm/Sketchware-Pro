...
The current service-call slice maps direct legacy operations to structured arguments for HTTP (`requestnetworkStartRequestNetwork`), TextToSpeech speak/configuration/lifecycle, speech recognition start, camera capture, document picking, gyroscope start/stop, vibrator duration, and basic dialog show. The mapping deliberately leaves stateful builder operations such as request-network headers/params and dialog title/button callbacks for the next callback/stateful-service slice rather than pretending they are already equivalent.
