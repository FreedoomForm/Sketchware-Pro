...
| Block and event models | `BlockBean`, `EventBean` | **R1 subset / visible R0 remainder** | `CreatorLegacyArtifactImporter` imports supported view events and block chains into `CreatorEventBinding`; `CreatorRuntimeBlock` now has typed nested state-equality branches | Importer tests cover typed message blocks, linked chains, and rejection of `executeJava`; control-flow execution and nested persistence regressions pass | Unsupported opcodes are reported as R0 and never delegated to code generation | **Open** — legacy opcode and activity-event coverage remains incomplete |
