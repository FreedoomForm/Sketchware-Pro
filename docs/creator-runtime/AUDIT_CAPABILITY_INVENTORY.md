...
| Resources/styles | `ProjectResourceBean`, XML builders, view/layout/image/text beans | Images, colors, dimensions, styles, manifest resources | R1 typed import/resolver evidence covers image/video/sound/font metadata plus strings/colors/styles/themes/arrays and qualifier retention; live visual/device parity and any non-allow-listed resource family remain explicit gates or visible R0. |
| Project files | `ProjectBean`, `ProjectFileBean`, storage managers | Activities, fragments, custom views, drawer, source/logic/resource joins | R1 Project IR preserves versioned screens, activities, layouts, drawer relationships, component descriptors, typed blocks, state, and resource metadata; codec/importer regressions pass. Non-activity legacy file behavior and live device parity remain explicit gates. |
| Build/export | `ProjectBuilder`, compiler and resource/dex pipelines | Debug/release APK, AAB, source generation, compiler diagnostics | Creator Runtime is deliberately runtime-native and does not execute a generated-project compiler tier. Debug APK/JVM/native workflow evidence exists for the host app; arbitrary source-generation/build delegation is a visible R0 boundary, not an R2/R3 fallback. |
| Editor/AI/recovery | Main/design/editor, AI tool registry, Creator Runtime | Human actions, agent parity, history, recovery, native handoff | R1 operation mapper/validator is shared by AI and live runtime paths; typed state, collection, event, revision, and serialization regressions pass. Legacy editor-only actions outside the typed operation contract remain visible, enumerated gates rather than implicit coverage. |
