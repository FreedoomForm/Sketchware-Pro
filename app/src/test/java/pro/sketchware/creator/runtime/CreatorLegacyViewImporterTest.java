...
public class CreatorLegacyViewImporterTest {
    ...
    @Test public void preservesLegacyTextFontStyleAndResourceColorProperties() {
        ...
        title.text.textFont = "display";
        title.text.textType = 1;
        title.text.resTextColor = "ink";
        title.text.resHintColor = "@color/muted";

        CreatorLegacyViewImporter.Result result = new CreatorLegacyViewImporter().importLayout(
                "project", "Imported", "main", "/", Arrays.asList(title));

        java.util.Map<String, Object> properties = result.getDocument().getWidgets().get("title").getProperties();
        assertThat(properties).containsEntry("textFont", "display");
        assertThat(properties).containsEntry("textType", 1);
        assertThat(properties).containsEntry("textColor", "@color/ink");
