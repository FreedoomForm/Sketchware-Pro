...
public class CreatorRuntimeOperationMapperTest {
    ...
    @Test public void preservesAiCollectionPropertiesAsTypedLists() {
        JsonObject args = new JsonObject();
        args.addProperty("action", "add_widget");
        args.addProperty("widget_id", "choices");
        args.addProperty("widget_type", "spinner");
        args.addProperty("parent_id", "root");
        JsonObject properties = new JsonObject();
        ...
        @SuppressWarnings("unchecked") java.util.Map<String, Object> mapped =
                (java.util.Map<String, Object>) operation.getPayload().get("properties");
        assertThat(mapped.get("items")).isInstanceOf(java.util.List.class);
        assertThat((java.util.List<?>) mapped.get("items")).containsExactly("one", "two").inOrder();
    }
}
