...
public final class CreatorFirebaseAuthService implements CreatorRuntimeService {
    ...
    @Override public Result execute(Map<String, Object> arguments) {
        ...
        if ("reset_password".equals(action)) {
            if (email == null) return CreatorRuntimeServiceArguments.invalid("reset_password requires email.");
            auth.sendPasswordResetEmail(email).addOnSuccessListener(ignored -> environment.publish(getId(), "password_reset",
                    CreatorRuntimeServiceArguments.output("email", email)))
                    .addOnFailureListener(error -> publishError(action, error));
            return CreatorRuntimeServiceArguments.succeeded("started", true, "action", action);
        }
