...
public final class CreatorRuntimeTool implements SketchwareTool {
    ...
    @Override public JsonObject jsonSchema() {
        ...
        addString(properties, "state_id", "Stable Creator Runtime state ID.");
        ...
        blocks.addProperty("description", "Ordered visible blocks. Every typed Creator Runtime block instrument is supported: set_widget_property, set_state, increment_state, list_mutate, map_mutate, attach_event, show_message, navigate, runtime_service_call, custom_function_call, return, if_state_equals, if_boolean, repeat, forever, and break. Conditional and loop blocks may contain nested then_blocks and else_blocks arrays.");
        ...
        blockType.addProperty("type", "string");
        JsonArray blockTypes = new JsonArray();
        blockTypes.add("set_widget_property");
        blockTypes.add("set_state");
        blockTypes.add("increment_state");
        ...
        blockTypes.add("custom_function_call");
        blockTypes.add("return");
        blockTypes.add("if_state_equals");
        blockTypes.add("if_boolean");
        blockTypes.add("repeat");
        blockTypes.add("forever");
        blockTypes.add("break");
        blockType.add("enum", blockTypes);
        blockProperties.add("type", blockType);
        blockItem.add("properties", blockProperties);
        blocks.add("items", blockItem);
